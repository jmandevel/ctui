...
const CTUI_ColorRgba32 CTUI_ANSI16_PALETTE[16] = {
    {0, 0, 0, 255},       // 0  Black
    {128, 0, 0, 255},     // 1  Red
    {0, 128, 0, 255},     // 2  Green
...
const uint8_t CTUI_CUBE555_CHANNELS[6] = {0, 95, 135, 175, 215, 255};

uint8_t CTUI_getNearestGrayChannel(CTUI_PaletteIndexAnsi256 ansi256) {
  return (uint8_t)(8 + ansi256 * 10);
}

uint8_t CTUI_getNearestAnsi256Cube555Channel(uint8_t channel) {
  if (channel < 48) // comparing with half way points between 6 regions
    return 0;
  if (channel < 115)
    return 1;
  if (channel < 155)
...
CTUI_ColorRgba32 CTUI_convertAnsi256ToRgba32(CTUI_PaletteIndexAnsi256 ansi256) {
  ...
  return (CTUI_ColorRgba32){gray, gray, gray, 255};
}

CTUI_PaletteIndexAnsi256 CTUI_convertRgba32ToAnsi256(CTUI_ColorRgba32 rgba32) {
  int r = CTUI_getNearestAnsi256Cube555Channel(rgba32.r);
  int g = CTUI_getNearestAnsi256Cube555Channel(rgba32.g);
  int b = CTUI_getNearestAnsi256Cube555Channel(rgba32.b);
  return CTUIP_ANSI256_CUBE(r, g, b);
}

CTUI_PaletteIndexAnsi8
CTUI_convertAnsi16ToAnsi8(CTUI_PaletteIndexAnsi16 ansi16) {
  return (CTUI_PaletteIndexAnsi8)((int)ansi16 & 7);
}

CTUI_PaletteIndexAnsi8
CTUI_convertAnsi256ToAnsi8(CTUI_PaletteIndexAnsi256 ansi256) {
  CTUI_ColorRgba32 rgb = CTUI_convertAnsi256ToRgba32(ansi256);
  return CTUI_convertRgba32ToAnsi8(rgb);
}

CTUI_PaletteIndexAnsi16
CTUI_convertAnsi256ToAnsi16(CTUI_PaletteIndexAnsi256 ansi256) {
  int ansi_i = (int)ansi256;
  if (ansi_i < 16) {
    return (CTUI_PaletteIndexAnsi16)ansi_i;
  }
  CTUI_ColorRgba32 rgb = CTUI_convertAnsi256ToRgba32(ansi256);
  return CTUI_convertRgba32ToAnsi16(rgb);
}

CTUI_PaletteIndexAnsi8 CTUI_convertRgba32ToAnsi8(CTUI_ColorRgba32 rgba32) {
  uint8_t r = rgba32.r, g = rgba32.g, b = rgba32.b;
  int best_i = 0;
  int best_dist = CTUI_getColorDistanceSquared(
      r, g, b, CTUI_ANSI16_PALETTE[0].r, CTUI_ANSI16_PALETTE[0].g,
      CTUI_ANSI16_PALETTE[0].b);
  for (int i = 1; i < 8; i++) {
    int d = CTUI_getColorDistanceSquared(r, g, b, CTUI_ANSI16_PALETTE[i].r,
                                         CTUI_ANSI16_PALETTE[i].g,
                                         CTUI_ANSI16_PALETTE[i].b);
    if (d < best_dist) {
      best_dist = d;
      best_i = i;
    }
  }
  return (CTUI_PaletteIndexAnsi8)best_i;
}

CTUI_PaletteIndexAnsi16 CTUI_convertRgba32ToAnsi16(CTUI_ColorRgba32 rgba32) {
  uint8_t r = rgba32.r, g = rgba32.g, b = rgba32.b;
  int bestIdx = 0;
  int bestDist = CTUI_getColorDistanceSquared(r, g, b, CTUI_ANSI16_PALETTE[0].r,
                                              CTUI_ANSI16_PALETTE[0].g,
                                              CTUI_ANSI16_PALETTE[0].b);
  ...
  return (CTUI_PaletteIndexAnsi16)bestIdx;
}
...
CTUI_PaletteIndexAnsi8 CTUI_convertToAnsi8(CTUI_Color color) {
  ...
  case CTUIC_ANSI16:
    return CTUI_convertAnsi16ToAnsi8(color.data.ansi16);
  case CTUIC_ANSI256:
    return CTUI_convertAnsi256ToAnsi8(color.data.ansi256);
  case CTUIC_RGBA32:
    return CTUI_convertRgba32ToAnsi8(color.data.rgba32);
  default:
    return CTUIP_ANSI8_WHITE;
  }
}
...
CTUI_PaletteIndexAnsi16 CTUI_convertToAnsi16(CTUI_Color color) {
  ...
  case CTUIC_ANSI16:
    return color.data.ansi16;
  case CTUIC_ANSI256:
    return CTUI_convertAnsi256ToAnsi16(color.data.ansi256);
  case CTUIC_RGBA32:
    return CTUI_convertRgba32ToAnsi16(color.data.rgba32);
  default:
    return CTUIP_ANSI16_WHITE;
  }
}
...
CTUI_PaletteIndexAnsi256 CTUI_convertToAnsi256(CTUI_Color color) {
  ...
  case CTUIC_ANSI16:
    return CTUI_convertAnsi16ToAnsi256(color.data.ansi16);
  case CTUIC_ANSI256:
    return color.data.ansi256;
  case CTUIC_RGBA32:
    return CTUI_convertRgba32ToAnsi256(color.data.rgba32);
  default:
    return CTUIP_ANSI256_WHITE;
  }
}
...
CTUI_ColorRgba32 CTUI_convertToRgba32(CTUI_Color color) {
  ...
  case CTUIC_ANSI16:
    return CTUI_convertAnsi16ToRgba32(color.data.ansi16);
  case CTUIC_ANSI256:
    return CTUI_convertAnsi256ToRgba32(color.data.ansi256);
  case CTUIC_RGBA32:
    return color.data.rgba32;
  default:
    return (CTUI_ColorRgba32){255, 255, 255, 255};
  }
}
